...
class ClickableSprite(pygame.sprite.Sprite):
    def __init__(self, image, callback):
        ...
        self.rect.x = round(int(player_pos.x), -2)
        self.rect.y = round(int(player_pos.y), -2)
        self.callback = callback

    def update(self, events):
        self.rect.x = int(player_pos.x)
        self.rect.y = int(player_pos.y)
        for event in events:
...
sprite = ClickableSprite(pygame.Surface((150, 150)), on_click)
